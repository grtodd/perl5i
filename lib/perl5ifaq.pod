...
=head2 Can I use the re.pl from Devel::REPL with perl5i?

Yes. If you add an rcfile to C<$HOME/.re.pl/perl5i> amd call C<re.pl> with
an configuration argument like this:
  
  re.pl --rcfile perl5i

then the statments in C<$HOME/.re.pl/perl5i> will be executed just as if you 
executed them in the C<re.pl> command shell. Here ahe contents of a working 
rcfile:
...
NB: C<use indirect ;> is a workaround for a current limitation in LexEnv.
